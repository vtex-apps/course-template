...
const correctionHandler = async (ctx: Context) => {
  ...
  const testCtx = {
    parseCss: (fileContent: string) => {
      const parsedCss = postcss.parse(fileContent)
      return postcssJs.objectify(parsedCss)
    },
    parseJsonc: (fileContent: string) => jsonc.parse(fileContent),
    ramda,
    getFile: (path: string) =>
